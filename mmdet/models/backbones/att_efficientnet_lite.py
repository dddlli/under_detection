...
from ..utils import act_layers, iAFF, AFF, MS_CAM, DAF
    ...
    def __init__(
        ...
        if self.has_se:
            ...
            self._se_expand = nn.Conv2d(
                ...
                kernel_size=1)
        if fuse_type == 'AFF':
            self.fuse_mode = AFF(channels=oup)
        elif fuse_type == 'iAFF':
            self.fuse_mode = iAFF(channels=oup)
        elif fuse_type == 'DAF':
            self.fuse_mode = DAF()
        else:
            self.fuse_mode = None
...
class AttEfficientNetLite(nn.Module):
    ...
    def __init__(self,
                 ...
                 activation="ReLU6",
                 pretrain=True):
        super(AttEfficientNetLite, self).__init__()
